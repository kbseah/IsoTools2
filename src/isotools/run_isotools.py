...
def argument_parser():
    parser = argparse.ArgumentParser(
        prog="isotools", description="process LRTS data with isotool"
    )
    parser.add_argument(
        "--anno",
    ...
    parser.add_argument(
        "--samples", metavar="<samples.tsv>", help="add samples from sample tsv"
    )
    parser.add_argument(
        "--file_prefix",
    ...
    parser.add_argument(
        "--file_suffix",
        metavar="<suffix>",
        help="Specify output sufix (not used for pickle).",
    )
    parser.add_argument(
        "--short_read_samples",
        metavar="<samples.csv>",
        help="Specify tsv with short read samples.",
    )
    parser.add_argument(
        "--force_recreate",
        help="reimport transcriptomes from alignments, even in presence of pickle file.",
        action="store_true",
    )
    parser.add_argument(
        "--no_pickle",
        help="Do not pickle the transcriptome for later use.",
        action="store_true",
    )
    parser.add_argument(
        "--progress_bar",
        help="Show the progress of individual tasks.",
        action="store_true",
    )
    parser.add_argument(
        "-l",
    ...
    parser.add_argument(
        ...
        type=str,
        default="png",
        choices=["png", "pdf", "svg", "eps", "pgf", "ps"],
    )
    parser.add_argument(
        "--plot_dpi",
        metavar="<dpi>",
        type=int,
        default=100,
        help="Specify resolution of plots",
    )
    parser.add_argument(
        "--altsplice_plots",
        metavar="<n>",
...
def main():
    ...
    if args.short_read_samples:
        ...
        isoseq.add_short_read_coverage(
            dict(zip(illu_samples["name"], illu_samples["file_name"]))
        )

    illu_groups = {}
    if (
        "short_reads" in isoseq.infos
    ):  # todo: make this optional/parameter --dont_use_short_reads
        ...
        logger.debug(
            "illumina sample group definition: %s\n%s",
            illu_groups,
    ...
    if args.gtf_out:
        ...
        isoseq.write_gtf(
            gtf_fn, query=args.filter_query, progress_bar=args.progress_bar
        )
    ...
    if args.qc_plots:
        filter_plots(
            isoseq,
            groups,
            f"{args.file_prefix}_filter_stats{file_suffix}.{args.plot_type}",
            args.progress_bar,
        )
        transcript_plots(
            isoseq,
            groups,
            f"{args.file_prefix}_transcript_stats{file_suffix}.{args.plot_type}",
            args.progress_bar,
        )
    ...
    if args.altsplice_stats:
        altsplice_plots(
            isoseq,
            groups,
            f"{args.file_prefix}_altsplice{file_suffix}.{args.plot_type}",
            args.progress_bar,
        )
    ...
    if args.samples:
        ...
        if isoseq is None:
            isoseq = Transcriptome.from_reference(
                args.anno, progress_bar=args.progress_bar
            )
        ...
        for _, row in sample_tab.iterrows():
            ...
            isoseq.add_sample_from_bam(
                fn=row.file_name, progress_bar=args.progress_bar, **sample_args
            )
            added = True
        if added:
...
def transcript_plots(isoseq: Transcriptome, groups, filename, progress_bar):
    ...
    tr_stats.append(
        isoseq.transcript_length_hist(
            ...
            add_reference=True,
            min_coverage=2,
            transcript_filter=dict(query="FSM", progress_bar=progress_bar),
        )
    )
    ...
    tr_stats.append(
        isoseq.downstream_a_hist(
            groups=groups,
            transcript_filter=dict(
                query="not (NOVEL_GENE or UNSPLICED)", progress_bar=progress_bar
            ),
            ref_filter=dict(query="not UNSPLICED"),
        )
    )
    ...
    tr_stats.append(
        isoseq.downstream_a_hist(
            groups=groups,
            transcript_filter=dict(
                query="NOVEL_GENE and UNSPLICED", progress_bar=progress_bar
            ),
        )
    )
    ...
    tr_stats.append(
        isoseq.direct_repeat_hist(
            groups=groups, transcript_filter=dict(progress_bar=progress_bar)
        )
    )
    tr_stats.append(
        (
            pd.concat(
                [
                    tr_stats[2][0].add_suffix(" novel unspliced"),
                    tr_stats[1][0].add_suffix(" known multiexon"),
                ],
                axis=1,
            ),
            tr_stats[2][1],
    ...
    isotools.plots.plot_bar(
        f_stats[0], ax=axs[0, 1], drop_categories=["PASS"], **f_stats[1]
    )
    # B) internal priming
    isotools.plots.plot_distr(
        tr_stats[4][0][[c for c in tr_stats[4][0].columns if "novel" in c]],
        smooth=3,
        ax=axs[1, 0],
        density=True,
        fill=True,
    ...
    isotools.plots.plot_distr(
        tr_stats[4][0][[c for c in tr_stats[4][0].columns if "known" in c]],
        smooth=3,
        ax=axs[1, 1],
        density=True,
        fill=True,
    ...
    # C) RTTS
    isotools.plots.plot_distr(
        tr_stats[3][0][[c for c in tr_stats[3][0].columns if "novel" in c]],
        ax=axs[2, 0],
        density=True,
        **tr_stats[3][1],
    )
    isotools.plots.plot_distr(
        tr_stats[3][0][[c for c in tr_stats[3][0].columns if "known" in c]],
        ax=axs[2, 1],
        density=True,
        **tr_stats[3][1],
    )
...
def altsplice_plots(isoseq: Transcriptome, groups, filename, progress_bar):
    ...
    altsplice = isoseq.altsplice_stats(
        groups=groups,
        transcript_filter=dict(
            query="not (RTTS or INTERNAL_PRIMING)", progress_bar=progress_bar
        ),
    )
    ...
    fig, ax = plt.subplots()
    isotools.plots.plot_bar(
        altsplice[0], ax=ax, drop_categories=["FSM"], **altsplice[1]
    )
    fig.tight_layout(rect=[0, 0, 1, 0.95])
    fig.savefig(filename)
...
def altsplice_examples(
    isoseq: Transcriptome, n, query="not FSM"
):  # return the top n covered genes for each category
    examples = {}
    for gene, transcript_ids, transcripts in isoseq.iter_transcripts(
        query=query, genewise=True
    ):
        ...
        for transcript_id, transcript in zip(transcript_ids, transcripts):
            ...
            for cat in transcript["annotation"][1]:
                examples.setdefault(cat, []).append(
                    (score, gene.name, gene.id, transcript_id, cov, total_cov)
                )
    ...
    for cat, best_list in examples.items():
        ...
        for i, (_score, gene_name, gene_id, transcript_id, cov, total_cov) in enumerate(
            ...
            try:
                ...
                info = list()
            logger.info(
                f"{i + 1}. best example for {cat}: {gene_name} {transcript_id} {info}, {cov} {total_cov} ({cov/total_cov:%})"
            )
            joi = []  # set joi
            ...
            if info:
                ...
                if cat == "exon skipping":
                    ...
                    for i, exon in enumerate(exons[1:-1]):
                        if exon in info:
                            junctions.extend(
                                [(exons[i][1], exon[0]), (exon[1], exons[i + 2][0])]
                            )
            ...
            if info:
                for pos in info:
                    ...
                    axs[0].set_title(
                        f"{gene.name} {gene.chrom}:{start}-{end} {cat} (cov={cov})"
                    )
...
def plot_diffsplice(
    isoseq: Transcriptome, de_tab, groups, illu_gr, file_prefix, plot_type
):
    ...
    for gene_id in de_tab["gene_id"].unique():
        ...
        for _, regOI in de_tab.loc[de_tab["gene_id"] == gene_id].iterrows():
            ...
            joi.extend(
                [
                    (exon1[1], exon2[0])
                    for exon1, exon2 in pairwise(
                        gene.transcripts[transcriptA[0]]["exons"]
                    )
                    if exon1[1] >= regOI.start and exon2[0] <= regOI.end
                ]
            )
            joi.extend(
                [
                    (exon1[1], exon2[0])
                    for exon1, exon2 in pairwise(
                        gene.transcripts[transcriptB[0]]["exons"]
                    )
                    if exon1[1] >= regOI.start and exon2[0] <= regOI.end
                ]
            )
        ...
        for _, row in de_tab.loc[de_tab.gene == gene.name].iterrows():
            if row.start > gene.start and row.end < gene.end:
                ...
                fig.savefig(
                    f'{file_prefix}_{"_".join(groups)}_{gene.name}_zoom_{row.start}_{row.end}_sashimi.{plot_type}'
                )
        plt.close(fig)
...
def test_differential(isoseq: Transcriptome, groups, illu_groups, args, file_suffix):
    ...
    for diff_cmp in args.diff:
        ...
        if len(gr) != 2:
            logger.error(
                '--diff argument format error: provide two groups separated by "/" -- skipping'
            )
            continue
        ...
        res = isoseq.altsplice_test(gr, progress_bar=args.progress_bar).sort_values(
            "pvalue"
        )
        sig = res.padj < 0.1
        logger.info(
